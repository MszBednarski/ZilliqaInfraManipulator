...
import { createValParam, deploy, getContractState } from "./utill";
 ...
 */
getCollectorState("zil12n666f9925le2zgewkrxn6u489562thl3gazeg");
...
async function getCollectorState(a: string) {
  const zil = await getZil();
  const state = await getContractState(zil, a);
  console.log(state);
}
